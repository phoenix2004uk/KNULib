...
local TRN is bundle(List("trn/transferMinmus","trn/capture")).
local matchInc is import("mnv/matchInc").
...
function matchMinmusInclination {
	set burn to matchInc(Minmus).
	mission["next"]().
}
function transferToMinmus {
	local res is TRN["transferMinmus"](25000).
	if res = "wait" {
